...
exports.addToCart = async (req, res) => {
  try {
    ...
    if (!bookDetail) return res.status(404).json({ message: "Không tìm thấy sản phẩm" });
    ...
    if (itemIndex > -1) {
      ...
      if (newQuantity > bookDetail.stock_quantity)
        return res.status(400).json({ message: `Tồn kho chỉ còn ${bookDetail.stock_quantity}` });
    ...
    res.status(200).json({ message: "Đã thêm vào giỏ", cart: formatCart(savedCart) });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
};
...
//     // 🌟 Nếu giỏ chưa tồn tại → tạo mới và reset thời gian + trạng thái thông báo
//     if (!cart) {
//       cart = new Cart({
//         user_id: userId,
//         items: [],
...
//     // 🌟 Nếu giỏ có tồn tại nhưng TRỐNG → reset timer
